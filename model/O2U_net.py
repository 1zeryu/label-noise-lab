...
import cifar_noise_data.datasets
import numpy as np
from torch.autograd import Variable
...
class O2U_net():
    ...
    def first_stage(self,filter_mask=None):
        if filter_mask is not None:
            dataset = Mask_Select(self.trainset,filter_mask) + Mask_Select(self.train_argumentation1,filter_mask) + Mask_Select(self.train_augumentation2,filter_mask)
        ...
        for epoch in range(1,self.n_epochs+1):
            ...
            for i,(images,labels,indexes) in enumerate(tqdm(train_loader_init,desc=desc)):
                ...
                loss_1.backward()
                optimizer1.step()
            print('epoch: {}, lr: {}, train_loss: {}, test_accuacy: {}'.format(epoch,lr,globals_loss/ndata,accuracy))
            # 保存文件
    ...
    def second_stage(self,forget_rate):
        ...
        for epoch in range(1,self.max_epochs+1):
            ...
            noise_accuracy = np.sum(self.noise_or_not[ind_1_sorted[num_remember:]]) / float(len(loss_1_sorted)-num_remember)
            ...
            top_accuracy = 1 - np.sum(self.noise_or_not[ind_1_sorted[top_accuracy_rm:]]) / float(len(loss_1_sorted) - top_accuracy_rm)
            print('epoch:{}, lr: {}, train_loss: {}, test_acc: {}, noise_accuracy: {}, top 0.1 noise_accuracy: {}'.format(epoch,lr,global_loss/ndata,accuracy,1-noise_accuracy,top_accuracy)) 
        return mask
